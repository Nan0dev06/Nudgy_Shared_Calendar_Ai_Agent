...
**Shared group events are not unilaterally editable — not even by whoever
created them.** A change to a shared event goes back to the group as a vote
(every field, titles included: a meeting renamed from "quarter goals" to
"week analysis" is exactly as material as a moved time). That mechanism
doesn't exist yet, so `can_edit` says no and says why; when it lands, this is
the function it hangs off.
...
_NEEDS_VOTE = (
    "Changes to a shared group event go to the group, not to one person. "
    "Propose the change and everyone votes on it."
)
...
def can_edit(event, user_id: int, creator_email: str | None = None) -> Decision:
    ...
    if event.personal:
        ...
        return ALLOWED
    # Shared: nobody edits directly, creator included. Not an oversight — see
    # the module docstring and docs/v1-decisions.md ("Editing events & tasks").
    return Decision(False, _NEEDS_VOTE)
